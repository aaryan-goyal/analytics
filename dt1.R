...
dtree1    #only one root node created...
dtree1 = rpart(buy ~ education + married, data=data,  minsplit=4, minbucket=2,cp=-1)  #cp=complexity factor 
dtree1
rpart.plot(dtree1, nn=T)   #nn=node number
printcp(dtree1)
