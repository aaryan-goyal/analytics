...
sum(is.na(v1))  # sum TRUE values ie missing values....count number of missing values
mean(v1)  # cannot calculate if NA values in vector
mean(v1, na.rm=T)  # remove NA values and the calc
...
v1[is.na(v1)]
v1[is.na(v1)] = mean(v1, na.rm=T)   #substituting mean in place of NA values
v1
...
#Tabulate
library(mice)    #to create a visual rep of missing values
